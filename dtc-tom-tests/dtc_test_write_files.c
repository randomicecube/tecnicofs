...
void check_if_file_was_correctly_written(char *input_file, char *tfs_file) {
    ...
    while (bytes_read_external > 0 && bytes_read_tfs > 0) {
        if (bytes_read_external != bytes_read_tfs) {
            
        }
        if (strncmp(buffer_external, buffer_tfs, BUFFER_LEN) != 0) {
            fwrite(buffer_external, sizeof(char), BUFFER_LEN, stdout);
            printf("\n");
            fwrite(buffer_tfs, sizeof(char), BUFFER_LEN, stdout);
            printf("\n");
        }
